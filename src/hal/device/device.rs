...
mod plugin_adapter {
    ...
    use crate::hal::facilities::{
        ...
        EventExtTriggerCallback, RawEventStreamDecoderFacility, RawEventStreamFacility,
        FacilityError, FacilityHandle, FacilityType, GeometryFacility, HWIdentificationFacility,
        ROIFacility, SensorInfo, StreamBuffer, SystemInfo,
    };
    use crate::hal::device::plugin::{DevicePluginBox, PluginFacilityHandle, PluginFacilityType};
    use std::collections::HashMap;
    use std::sync::{Arc, Mutex};
    use abi_stable::{std_types::{RSlice, RString}, type_level::downcasting::TD_Opaque};
    ...
    impl PluginDevice {
        ...
        fn native_handle(&self, kind: PluginFacilityType) -> Option<FacilityHandle> {
            ...
            match handle {
                PluginFacilityHandle::Geometry(geometry) => Some(
                    FacilityHandle::GeometryFacility(Arc::new(PluginGeometryFacility { geometry })),
                ),
                PluginFacilityHandle::Roi(roi) => Some(FacilityHandle::ROIFacility(Arc::new(
                    std::sync::RwLock::new(PluginRoiFacility { roi }),
                ))),
                PluginFacilityHandle::RawEventStream(stream) => Some(
                    FacilityHandle::RawEventStreamFacility(Arc::new(std::sync::RwLock::new(
                        PluginRawEventStreamFacility { stream },
                    ))),
                ),
                PluginFacilityHandle::HardwareIdentification(identity) => Some(
                    FacilityHandle::HWIdentificationFacility(Arc::new(
                        PluginHardwareIdentificationFacility { identity },
                    )),
                ),
                PluginFacilityHandle::RawEventStreamDecoder(decoder) => Some(
                    FacilityHandle::RawEventStreamDecoderFacility(Arc::new(std::sync::RwLock::new(
                        PluginRawEventStreamDecoderFacility { decoder },
                    ))),
                ),
                PluginFacilityHandle::EventSubscription(decoder) => Some(
                    FacilityHandle::EventSubscriptionFacility(Arc::new(std::sync::RwLock::new(
                        PluginEventSubscriptionFacility { decoder },
                    ))),
                ),
                _ => None,
            }
    ...
    impl ROIFacility for PluginRoiFacility {
        fn get_enabled(&self) -> crate::hal::facilities::FacilityResult<bool> {
            self.roi.get_enabled().into_result().map_err(|e| FacilityError::Plugin(e.to_string()))
        }
        fn set_enabled(&mut self, value: bool) -> crate::hal::facilities::FacilityResult<()> {
            self.roi.set_enabled(value).into_result().map_err(|e| FacilityError::Plugin(e.to_string()))
        }
        fn set_roi(&mut self, region: crate::hal::types::Region) -> crate::hal::facilities::FacilityResult<()> {
            let region = crate::hal::device::plugin::PluginRegion {
                x: region.0, y: region.1, width: region.2, height: region.3,
            };
            self.roi.set_roi(region).into_result().map_err(|e| FacilityError::Plugin(e.to_string()))
        }
        fn set_rois(&mut self, regions: &[crate::hal::types::Region]) -> crate::hal::facilities::FacilityResult<()> {
            let regions: Vec<_> = regions.iter().map(|region| crate::hal::device::plugin::PluginRegion {
                x: region.0, y: region.1, width: region.2, height: region.3,
            }).collect();
            self.roi.set_rois(regions.as_slice().into()).into_result().map_err(|e| FacilityError::Plugin(e.to_string()))
        }
        fn roi(&self) -> Option<crate::hal::types::Region> {
            self.roi.roi().into_option().map(|r| (r.x, r.y, r.width, r.height))
        }
        fn rois(&self) -> Option<Vec<crate::hal::types::Region>> {
            let regions = self.roi.rois();
            if regions.is_empty() { None } else { Some(regions.into_iter().map(|r| (r.x, r.y, r.width, r.height)).collect()) }
        }
    }
    ...
    impl EventSubscriptionFacility for PluginEventSubscriptionFacility {
        fn subscribe_to_cd_events(&mut self, callback: EventCDCallback) -> crate::hal::facilities::FacilityResult<()> {
            let sink = crate::hal::device::plugin::EventBatchSink_TO::from_value(
                PluginEventSink { cd: Some(Mutex::new(callback)), ext: None }, TD_Opaque,
            );
            self.decoder.subscribe_to_cd_events(sink).into_result().map_err(|e| FacilityError::Plugin(e.to_string()))
        }
        fn subscribe_to_ext_events(&mut self, callback: EventExtTriggerCallback) -> crate::hal::facilities::FacilityResult<()> {
            let sink = crate::hal::device::plugin::EventBatchSink_TO::from_value(
                PluginEventSink { cd: None, ext: Some(Mutex::new(callback)) }, TD_Opaque,
            );
            self.decoder.subscribe_to_ext_events(sink).into_result().map_err(|e| FacilityError::Plugin(e.to_string()))
        }
    }
    ...
    impl crate::hal::device::plugin::PluginDecoderErrorSink for PluginDecoderErrorSink {
        fn on_error(&self, message: RString) {
            let error: crate::hal::errors::SharedError = Arc::new(std::io::Error::other(message.to_string()));
    ...
    impl RawDecoderFacility for PluginRawEventStreamDecoderFacility {
        fn subscribe_to_protocol_violation(&mut self, callback: DecoderErrorCallback) -> crate::hal::facilities::FacilityResult<()> {
            let sink = crate::hal::device::plugin::PluginDecoderErrorSink_TO::from_value(
                PluginDecoderErrorSink { callback: Mutex::new(callback) }, TD_Opaque,
            );
            self.decoder.subscribe_to_protocol_violation(sink).into_result().map_err(|e| FacilityError::Plugin(e.to_string()))
        }
    ...
    impl RawEventStreamDecoderFacility for PluginRawEventStreamDecoderFacility {
        fn decode(&mut self, raw_data: &[u8]) -> crate::hal::facilities::FacilityResult<()> {
            self.decoder.decode(raw_data.into()).into_result().map_err(|e| FacilityError::Plugin(e.to_string()))
        }
        fn get_last_timestamp(&self) -> usize { self.decoder.get_last_timestamp() }
        fn get_timestamp_shift(&self) -> Option<usize> { self.decoder.get_timestamp_shift().into_option() }
        fn is_time_shifting_enabled(&self) -> bool { self.decoder.is_time_shifting_enabled() }
        fn reset_last_timestamp(&mut self, timestamp: usize) { self.decoder.reset_last_timestamp(timestamp); }
        fn reset_timestamp_shift(&mut self, shift: usize) { self.decoder.reset_timestamp_shift(shift); }
        fn is_decoded_event_stream_indexable(&self) -> bool { self.decoder.is_decoded_event_stream_indexable() }
    }
    ...
    impl HWIdentificationFacility for PluginHardwareIdentificationFacility {
        fn get_serial(&self) -> crate::hal::facilities::FacilityResult<String> {
            self.identity.get_serial().into_result().map(Into::into).map_err(|e| FacilityError::Plugin(e.to_string()))
        }
        fn get_system_id(&self) -> crate::hal::facilities::FacilityResult<i64> {
            self.identity.get_system_id().into_result().map_err(|e| FacilityError::Plugin(e.to_string()))
        }
        fn get_sensor_info(&self) -> crate::hal::facilities::FacilityResult<SensorInfo> {
            self.identity.get_sensor_info().into_result().map(|info| SensorInfo {
                name: info.name.into(), integrator: info.integrator.into(), version: info.version.into(),
            }).map_err(|e| FacilityError::Plugin(e.to_string()))
        }
        fn get_system_info(&self) -> crate::hal::facilities::FacilityResult<SystemInfo> {
            self.identity.get_system_info().into_result().map(|info| SystemInfo {
                serial_number: info.serial_number.into(), firmware_version: info.firmware_version.into(),
            }).map_err(|e| FacilityError::Plugin(e.to_string()))
        }
        fn get_connection_type(&self) -> crate::hal::facilities::FacilityResult<crate::hal::facilities::ConnectionType> {
            self.identity.get_connection_type().into_result().map(|kind| match kind {
                crate::hal::device::discovery::ConnectionType::Usb => crate::hal::facilities::ConnectionType::Usb,
                crate::hal::device::discovery::ConnectionType::Mipi => crate::hal::facilities::ConnectionType::Mipi,
                crate::hal::device::discovery::ConnectionType::Proprietary => crate::hal::facilities::ConnectionType::Proprietary,
                _ => crate::hal::facilities::ConnectionType::Unknown,
            }).map_err(|e| FacilityError::Plugin(e.to_string()))
        }
        fn get_available_data_encoding_formats(&self) -> crate::hal::facilities::FacilityResult<Vec<String>> {
            self.identity.get_available_data_encoding_formats().into_result()
                .map(|formats| formats.into_iter().map(Into::into).collect())
                .map_err(|e| FacilityError::Plugin(e.to_string()))
        }
        fn get_current_data_encoding_format(&self) -> crate::hal::facilities::FacilityResult<String> {
            self.identity.get_current_data_encoding_format().into_result().map(Into::into)
                .map_err(|e| FacilityError::Plugin(e.to_string()))
        }
    }
    ...
    impl RawEventStreamFacility for PluginRawEventStreamFacility {
        fn start(&mut self) -> crate::hal::facilities::FacilityResult<()> {
            self.stream.start().into_result().map_err(|e| FacilityError::Plugin(e.to_string()))
        }
        fn stop(&mut self) -> crate::hal::facilities::FacilityResult<()> {
            self.stream.stop().into_result().map_err(|e| FacilityError::Plugin(e.to_string()))
        }
        fn poll_buffer(&mut self) -> crate::hal::facilities::FacilityResult<StreamBuffer> {
            self.stream.poll_buffer().into_result().map(|b| (b.data.into(), b.valid_len)).map_err(|e| FacilityError::Plugin(e.to_string()))
        }
        fn wait_next_buffer(&mut self) -> crate::hal::facilities::FacilityResult<StreamBuffer> {
            self.stream.wait_next_buffer().into_result().map(|b| (b.data.into(), b.valid_len)).map_err(|e| FacilityError::Plugin(e.to_string()))
        }
    }
            ...
            self.registered
                ...
                .or_else(|| {
                    let kind = match facility_type {
                        ...
                        FacilityType::HWIdentificationFacility => PluginFacilityType::HardwareIdentification,
                        FacilityType::RawEventStreamDecoderFacility => PluginFacilityType::RawEventStreamDecoder,
                        FacilityType::EventSubscriptionFacility => PluginFacilityType::EventSubscription,
                        _ => return None,
                    };
            ...
            self.plugin
                ...
                .filter_map(|kind| {
                    let native = match kind {
                        ...
                        PluginFacilityType::HardwareIdentification => FacilityType::HWIdentificationFacility,
                        PluginFacilityType::RawEventStreamDecoder => FacilityType::RawEventStreamDecoderFacility,
                        PluginFacilityType::EventSubscription => FacilityType::EventSubscriptionFacility,
                        _ => return None,
                    };
